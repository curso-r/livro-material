...
Quando precisamos usar o mesmo `aes()` em vários `geoms`, podemos definí-lo dentro da função `ggplot()`. Esse `aes()` será então distribuído para todo `geom` do gráfico. O código anterior pode ser reescrito da seguinte forma:
