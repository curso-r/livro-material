...
Funções são tão comuns (provavelmente você já usou funções no Excel) e intuitivas, que mesmo sem definir o que elas são, nós já utilizamos funções nas seções anteriores:

- a função `c()` foi utilizada para criar vetores;
- a função `class()` foi utilizada para descobrir a classe de um objeto;
- a classe de funções `is.na()`, `is.nan()`, `is.infinite()` e `is.null` foram utilizadas para testar se um valor é `NA`, `NaN`, infinito ou `NULL`, respectivamente.
...
Também podemos usar argumentos para modificar o comportamento de uma função. Por exemplo, vejamos a função `mean()`.

```{r}
mean(1, 3)
```

Como você deve ter percebido, essa função retorna a média de seus argumentos. Também podemos passar um vetor como argumento, e ela retornará a média dos elementos do vetor.

```{r}
mean(c(1, 3))
```

Isso é útil pois podemos passar a coluna de um *data frame* como argumento para saber a média dessa coluna:

```{r}
# A coluna de um data frame é um vetor:
mtcars$mpg
...
Agora, o que acontece se algum elemento do vetor for `NA`?
